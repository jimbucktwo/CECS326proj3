...
// satisfies request if it is in a safe state
...
int main(int argc, char *argv[]) {
    ...
    // initiates available array to values inputted
    for (int i = 0; i < num_resources; i++) {
    ...
    // initiates maximum array to values in the text file
    for (int i = 0; i < num_customers; i++){
        for(int j = 0; j < num_resources; j++) {
    ...
    // main logic. Continues until the user terminates the program, takes in user input to RQ or RL
    while (valid) {
    ...
    if (commands[0] == "RQ") {
        ...
        // runs the safety algorithm to determine whether to complete the request
